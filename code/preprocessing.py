...
from utils.utils import numerize, filter_triplets, split_train_test_proportion
...
parser.add_argument('--genre_data', type=str, default='/opt/ml/input/GoogleNews-vectors-negative300.bin', help='Movielens dataset location')
parser.add_argument('--lr', type=float, default=1e-4, help='initial learning rate')
parser.add_argument('--wd', type=float, default=0.00, help='weight decay coefficient')
parser.add_argument('--batch_size', type=int, default=500, help='batch size')
parser.add_argument('--epochs', type=int, default=20, help='upper epoch limit')
parser.add_argument('--fold_size', type=int, default=3000, help='size of fold for evaluation')
...
parser.add_argument('--numpy_seed', type=int, default=98765, help='numpy random seed')
...
vad_plays_tr, vad_plays_te = split_train_test_proportion(vad_plays, numpy_seed=args.numpy_seed)

test_plays = raw_data.loc[raw_data['user'].isin(te_users)]
test_plays = test_plays.loc[test_plays['item'].isin(unique_sid)]
test_plays_tr, test_plays_te = split_train_test_proportion(test_plays, numpy_seed=args.numpy_seed)

train_data = numerize(train_plays, profile2id, show2id)
train_data.to_csv(os.path.join(args.data, 'train.csv'), index=False)

vad_data_tr = numerize(vad_plays_tr, profile2id, show2id)
vad_data_tr.to_csv(os.path.join(args.data, 'validation_tr.csv'), index=False)

vad_data_te = numerize(vad_plays_te, profile2id, show2id)
vad_data_te.to_csv(os.path.join(args.data, 'validation_te.csv'), index=False)

test_data_tr = numerize(test_plays_tr, profile2id, show2id)
test_data_tr.to_csv(os.path.join(args.data, 'test_tr.csv'), index=False)

test_data_te = numerize(test_plays_te, profile2id, show2id)
test_data_te.to_csv(os.path.join(args.data, 'test_te.csv'), index=False)
