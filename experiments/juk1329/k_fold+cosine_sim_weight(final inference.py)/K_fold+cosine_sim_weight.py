...
with torch.no_grad():
    for start_idx in tqdm(range(0, N, args.batch_size)):
        ...
        #===== cosine similarity을 반영한 가중치 조정(recvae와 hvamp) =====
        recon_batch = recon_batch_recvae#임시 recon_batch 설정
        ...
        #==== cosine similarity을 반영한 가중치 조정(ease와 recvae+hvamp) ====
        ...
        for user in np.array(idxlist[start_idx:end_idx])-start_idx:
            ...
            if user == 0:
                ...
                print("sim_final\n",sim_final)
            recon_batch[user] = recon_batch[user]*(0.5 + sim_final[0][0]) + ease_batch[user]*(1+sim_final[0][1])
