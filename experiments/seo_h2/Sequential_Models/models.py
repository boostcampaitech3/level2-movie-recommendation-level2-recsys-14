...
class S3RecModel(nn.Module):
    def __init__(self, args):
        ...
        self.position_embeddings = nn.Embedding(args.max_seq_length, args.hidden_size) # positional encoding은 성능 잘 안나옴
        self.item_encoder = Encoder(args) # transformer의 디코더 중 일부구성요소를 사용, 이미 bert구조로 구현되어있음
        self.LayerNorm = LayerNorm(args.hidden_size, eps=1e-12)
        self.dropout = nn.Dropout(args.hidden_dropout_prob)
        self.args = args

        # add unique dense layer for 4 losses respectively, pretrain
        self.aap_norm = nn.Linear(args.hidden_size, args.hidden_size) # 영화의 장르 이해, 장르 정답을 맞추도록
        self.mip_norm = nn.Linear(args.hidden_size, args.hidden_size) # 주변정보 학습(영화 중간을 비워놓고 이걸 영화로 맞추도록), 내부구조는 transformer인데 자기자신을 예측하도록 하니까 인코더라고 했음
        self.map_norm = nn.Linear(args.hidden_size, args.hidden_size) # 중간에 비어있는 칸의 장르를 예측하자
        self.sp_norm = nn.Linear(args.hidden_size, args.hidden_size) # 두개를 비워놓고 최종값에 context정보가 담겨져있다고 믿음 ex) 마지막에 어벤저스를 봤다는 것은 이걸알고있다는 것..?
    ...
    def associated_attribute_prediction(self, sequence_output, attribute_embedding):
        ...
        """
        sequence_output = self.aap_norm(sequence_output)  # [B L H] 영화를 집어넣으면 attribute의 임베딩 도출
    ...
    def masked_item_prediction(self, sequence_output, target_item):
        ...
        return torch.sigmoid(torch.sum(score, -1))  # [B*L] 빈영화에 대한 예측
    ...
    # Fine tune
    # same as SASRec, look-ahead 마스크 여기서 구현
...
# Bert모델로 SAS-Rec과 구조 유사, finetune 부분만 수정
class Bert4Rec(nn.Module):
    ...
    # Fine tune
    # same as SASRec, Bert4Rec 마스크 여기서 구현 - sasrec과 달리 양방향이므로 패딩 부분만 마스킹
    def finetune(self, input_ids): 
        ...
        sequence_output = item_encoded_layers[-1] # 마지막 레이어만
        return sequence_output # (B,L,H)
