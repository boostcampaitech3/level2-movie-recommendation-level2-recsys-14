...
parser.add_argument('--dataset', default = '/opt/ml/input/data/train/pro_sg', type=str)
...
def run(model, opts, train_data, batch_size, n_epochs, beta, gamma, dropout_rate):
    ...
    for epoch in range(n_epochs):
        for batch in generate(batch_size=batch_size, device=device, data_in=train_data, shuffle=True):
            ...
            #------------------------------------
            # print('ratings.size() :', ratings.size())
            #------------------------------------
...
# optimizer_encoder = adabound.AdaBound(encoder_params, lr=args.lr, final_lr=0.001)
# optimizer_decoder = adabound.AdaBound(decoder_params, lr=args.lr, final_lr=0.001)
...
for epoch in range(args.n_epochs):
    ...
    print(f"stop : {stop_cnt}/{early_stop}")

    wandb.log({
        "valid ndcg@100": valid_scores[-1],
        "train ndcg@100" : train_scores[-1]})
