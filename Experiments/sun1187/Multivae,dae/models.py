...
class MultiDAE_item_emb(nn.Module):
    def __init__(self, p_dims, q_dims=None, dropout=0.5):
        super(MultiDAE, self).__init__()
        item_genre_emb = pd.read_csv('item_genre_eb.csv')
        item_genre_emb.drop('Unnamed: 0', axis=1, inplace=True)
    ...
    def forward(self, input):
        ...
        h = F.normalize(input)
        h = self.drop(h)
        h = torch.cat((self.item_genre.to(args.device), h), 0) ###추가
        for i, layer in enumerate(self.layers):
            h = layer(h)
            if i != len(self.layers) - 1:
                h = F.tanh(h) 
...
class MultiVAE_item_emb(nn.Module):
    def __init__(self, p_dims, q_dims=None, dropout=0.5):
        super(MultiVAE, self).__init__()
