...
def main():
    ...
    args.model = 'bert'
    user_seq, max_item, valid_rating_matrix, test_rating_matrix, _ = get_user_seqs(args
    )

    item2attribute, attribute_size = get_item2attribute_json(item2attribute_file)
    
    args.max_item= max_item
    ...
    args.checkpoint_path = os.path.join(args.output_dir, checkpoint)
        # wandb설정
    wandb.init(project= 'movierec-project', reinit=True,
            config={"batch_size": args.batch_size,
            "lr"        : args.lr,
            "epochs"    : args.epochs,
            "name"      : args.model_name,
            "pretrain" : args.using_pretrain
                })
    ...
    if args.using_pretrain:
        ...
        try:
            ...
            print(f"{pretrained_path} Not Found! The Model is same as Bert4Rec")
    else:
        print("Not using pretrained model. The Model is same as Bert4Rec")
    ...
    for epoch in range(args.epochs):
        ...
        wandb.log({ "RECALL@5": scores[0],
            ...
            "RECALL@10":scores[2],
            "NDCG@10": scores[3]})
        early_stopping(np.array([scores[-2]]), trainer.model) #recall@10기준으로 저장하도록
