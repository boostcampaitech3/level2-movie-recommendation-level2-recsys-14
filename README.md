...
- 이번 프로젝트에서 실험한 단일 모델 종류는 Autoencoder 기반의 Multi-VAE, Multi-DAE, RecVAE, H+vamp, EASE를 사용했으며, 단일 모델 간 성능을 고려하여 최종 결과에 사용한 모델은 RecVAE, H+vamp, EASE입니다.
  - SASRec, BERT4Rec, S3Rec은 sequential data에 강한 모델로 알려져 있지만, 이번 프로젝트의 데이터는 단순 sequential data가 아니어서 예상보다 좋지 않은 결과가 나왔습니다. 그래서 좀 더 general recommendation에 적합한 모델을 탐색했습니다.
  ...
  - EASE(Embarrassingly Shallow Autoencoders for Sparse Data)는 파라미터의 해를 구하는 것이 closed form solution이며, 유저의 수가 증가할수록 loss function을 최소화하는 파라미터를 구하는 과정에서 쓰이는 Gram matrix의 계수가 커져서 파라미터를 구하는 error가 줄어들어 cold start problem에 강한 모델로 알려져 있습니다.
